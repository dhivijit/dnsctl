...
class AccountsScreen(Screen):
    ...
    BINDINGS = [
        ...
        Binding("n", "add_account", "Add"),
        Binding("d", "remove_account", "Remove"),
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label(
            "Enter: switch  N: add  D: remove  Esc: back",
            id="accounts-status",
        )
    ...
    def action_add_account(self) -> None:
        self._run_add_account()

    @work
    async def _run_add_account(self) -> None:
        from dnsctl.core.security import get_cached_password
        from dnsctl.tui.screens.auth import LoginScreen

        cached_pw = get_cached_password(self._current_alias)
        result = await self.app.push_screen_wait(
            LoginScreen(reuse_password=cached_pw)
        )
        if result is None:
            return
        new_alias, new_token = result
        self._current_alias = new_alias
        self._token = new_token
        self._load_accounts()
        self.query_one("#accounts-status", Label).update(
            f"[green]Account '{new_alias}' added.[/green]  Enter: switch  N: add  D: remove  Esc: back"
        )

    # ------------------------------------------------------------------ remove account

    def action_remove_account(self) -> None:
        self._run_remove_account()

    @work
    async def _run_remove_account(self) -> None:
        ...
        if len(accounts) <= 1:
            self.query_one("#accounts-status", Label).update(
                "[red]Cannot remove the last account.[/red]"
            )
            return
        ...
        confirmed = await self.app.push_screen_wait(
            ConfirmScreen(
                f"Remove account '{label}' ({alias})?\n\nAll local zone data will be deleted.",
        ...
        if alias == self._current_alias:
            ...
            from dnsctl.core.security import get_token
            new_token = get_token(new_alias) or ""
            self._relaunch_dashboard(new_alias, new_token)
            return

        self._load_accounts()
        self.query_one("#accounts-status", Label).update(
            f"[green]Removed '{alias}'.[/green]  Enter: switch  N: add  D: remove  Esc: back"
        )
