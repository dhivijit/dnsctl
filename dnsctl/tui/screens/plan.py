"""Plan screen — show drift, pending changes, and apply them."""
...
class PlanScreen(Screen):
    """Shows drift + planned local changes, with Apply and Force Apply."""
    ...
    def _populate_plan(self, plan) -> None:
        ...
        drift_info = self.query_one("#drift-info", Static)
        apply_btn = self.query_one("#apply-btn", Button)
        force_btn = self.query_one("#force-apply-btn", Button)
        table.clear()
        drift_info.update("")
        ...
        if plan.drift and plan.drift.has_changes:
            drift = plan.drift
            parts: list[str] = [f"[yellow bold]Drift detected: {drift.summary}[/yellow bold]"]
            for r in drift.added:
                parts.append(
                    f"  [green]+ {r.get('type','')} {r.get('name','')} → {r.get('content','')}[/green]"
                )
            for m in drift.modified:
                b, a = m["before"], m["after"]
                parts.append(
                    f"  [yellow]~ {b.get('type','')} {b.get('name','')}: "
                    f"{b.get('content','')} → {a.get('content','')}[/yellow]"
                )
            for r in drift.removed:
                parts.append(
                    f"  [red]- {r.get('type','')} {r.get('name','')} → {r.get('content','')}[/red]"
                )
            parts.append("[dim]Run Sync to pull these remote changes into local state.[/dim]")
            drift_info.update("\n".join(parts))
        ...
        if not plan.has_changes:
            ...
            if plan.drift and plan.drift.has_changes:
                msg += "  Sync first to resolve drift."
            status.update(msg)
            return
        ...
        if n_protected:
            summary += f"  ({n_protected} protected — use Force Apply to override)"
        status.update(summary)
        apply_btn.disabled = False

        # Show Force Apply button when protected records are in the plan
        if n_protected:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "back-btn":
            ...
            self._on_apply(force=False)
        elif event.button.id == "force-apply-btn":
            self._on_apply(force=True)
    ...
    def _on_apply(self, force: bool = False) -> None:
        ...
        self.query_one("#force-apply-btn", Button).disabled = True
        self._run_apply(force)

    @work(thread=True)
    def _run_apply(self, force: bool) -> None:
        self.app.call_from_thread(self._set_applying, True)
        try:
            from dnsctl.core.sync_engine import SyncEngine

            engine = SyncEngine(alias=self._alias)
            result = engine.apply_plan(self._plan, self._token, force=force)
        except Exception as exc:
            ...
            self.app.call_from_thread(self._restore_buttons)
            return
    ...
    def _restore_buttons(self) -> None:
        ...
        force_btn = self.query_one("#force-apply-btn", Button)
        if not force_btn.has_class("hidden"):
            force_btn.disabled = False
    ...
    def _show_apply_result(self, result) -> None:
        ...
        if result.all_succeeded:
            status.update(
                f"[green]Applied {len(result.succeeded)} change(s) successfully.[/green]"
            )
        else:
            status.update(
        ...
        # Regenerate plan to reflect applied state
        self._plan = None
        self.query_one("#plan-table", DataTable).clear()
        self.query_one("#drift-info", Static).update("")
        self.query_one("#force-apply-btn", Button).add_class("hidden")
        self._generate_plan()
